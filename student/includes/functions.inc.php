<?php
    ...
    function lrnInvalid($lrn) {
        $result;
        $lrnLength = strlen($lrn);

        if(($lrnLength < 12) || ($lrnLength > 12)) {
            $result = true;  
        } else {
            $result = false;
        }
    ...
    function createUser($conn, $year_level_id, $lrn, $firstname, $lastname, $middlename, $gender, $birthdate, $email, $password, $vkey) {

        $sql = "INSERT INTO tbl_student (year_level_id, student_lrn, first_name, last_name, middle_name, gender, birthdate, email, password, verification_key) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
        ...
        mysqli_stmt_bind_param($stmt, "isssssssss", $year_level_id, $lrn, $firstname, $lastname, $middlename, $gender, $birthdate, $email, $hashedPassword, $vkey);
